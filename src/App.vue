...
// [x] 8) При удалении тикера, не удаляется из localeStorage / Критичность: 4
...
// [] 10) Магические строки ичисла (url, 5000 мс задержки, ключ api, количество тикеров на странице) / Критичность: 1
// [x] 11) При переходе на вторую страницу добавляет "1" к page и получается page = 11

// [x] График сломан, если везде одинаковые значения
// [x] При удалении тикера остается выбор графика
...
export default {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=c7ad2664a028a12457f00dc9fabf5d26fcb0043489b90e8a28602840f173243d`
        )
